    ...
    if (search.value != "" && search.value != null) {
      ...
      return fetch(maRequete).then(function (reponse) {
        ...
        for (let i = 0; i < reponse["results"].length; i++) {
          html += '<tr> <td style="width: 25%; height: auto;"id="img"><img style="width : 100%" src="https://image.tmdb.org/t/p/w500' + reponse["results"][i]["poster_path"] + '"/></td><td valign="Top"><a class="notA" href="/Movie/' + reponse["results"][i]["id"] + '"> <h3 style="margin: 0px;">' + reponse["results"][i]["title"] + '</h3>' + ' Rating : ' + reponse["results"][i]["vote_average"] + '/10 <h6 style="margin: 0px;">' + reponse["results"][i]["overview"] + '</h6>';
    ...
    db.transaction(function (tx) {
      tx.executeSql('SELECT Is_Favorite FROM Tbl_Movies WHERE Id_Movie = ' + idMovie, [], function (tx, results) {
        if (results.rows[0] != null && results.rows[0]["Is_Favorite"] == 1) {
          html += '<button class=' + idMovie + ' onclick="' + "AddOrRemoveFavorite(" + idMovie + ",0,this)"+ '" type="button" style="margin-left: 90%; width: 10%; padding : 0%;background: transparent; border: none !important;"><i class="icon material-icons if-md">"star"</i></button> </a></td> </tr>'
        } else {
          html += '<button class=' + idMovie + ' onclick="' + "AddOrRemoveFavorite(" + idMovie + ",1,this)"+ '" type="button" style="margin-left: 90%; width: 10%; padding : 0%;background: transparent; border: none !important;"><i class="icon material-icons if-md">"star_border"</i></button> </a></td> </tr>'
        }
